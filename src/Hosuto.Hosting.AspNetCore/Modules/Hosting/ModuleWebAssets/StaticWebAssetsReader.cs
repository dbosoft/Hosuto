// based on https://github.com/dotnet/aspnetcore/blob/main/src/Hosting/Hosting/src/StaticWebAssets/StaticWebAssetsReader.cs
    ...
    internal static class StaticWebAssetsReader
    {
        ...
        internal static IEnumerable<ContentRootMapping> Parse(Stream manifest)
        {
            ...
            foreach (var element in document.Root.Elements())
            {
                ...
                {
                    throw new InvalidOperationException($"Invalid manifest format. Invalid element '{element.Name.LocalName}'. All {ModuleWebAssetsLoader.StaticWebAssetsManifestName} child elements must be '{ContentRootElementName}' elements.");
                }
                if (!element.IsEmpty)
                {
